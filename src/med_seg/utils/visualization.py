...
def plot_training_history(
    ...
    if 'val_loss' in history:
        ...
        axes[0].plot(best_epoch, best_loss, 'rx', markersize=12,
                    label='Best Model', markeredgewidth=2)
    ...
    if metric_key:
        ...
        if val_key in history:
            ...
            axes[1].plot(best_epoch, best_metric, 'rx', markersize=12,
                        label='Best Model', markeredgewidth=2)

        axes[1].set_xlabel('Epoch', fontsize=12)
        axes[1].set_ylabel(metric_key.capitalize(), fontsize=12)
        axes[1].set_title(f'Training and Validation {metric_key.capitalize()}',
...
def plot_dice_threshold_analysis(
    ...
    plt.plot(thresholds[best_idx], dice_scores[best_idx], 'r*',
            markersize=20, label=f'Best: {thresholds[best_idx]:.2f} (DICE={dice_scores[best_idx]:.4f})')
