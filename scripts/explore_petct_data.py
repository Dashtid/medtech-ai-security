...
def print_volume_info(name, info):
    ...
    print(f"    Spacing: {info['spacing'][0]:.2f}x{info['spacing'][1]:.2f}x{info['spacing'][2]:.2f} mm")
...
def explore_patient(patient_dir):
    ...
    if 'SEG' in volumes:
        ...
        print(f"    Tumor voxels: {seg_stats['tumor_voxels']:,} ({seg_stats['tumor_percent']:.3f}%)")

        if seg_stats['bbox']:
            bbox = seg_stats['bbox']
            print(f"    Tumor bounding box (z,y,x):")
    ...
    if len(volumes) > 1:
        ...
        if len(unique_shapes) == 1:
            ...
            print(f"    [!] WARNING: Volumes have different shapes!")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Explore PET/CT dataset structure"
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        "--patient",
        type=str,
        help="Specific patient to explore (default: all)"
    )
