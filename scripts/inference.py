...
def save_prediction_as_nifti(
    prediction: np.ndarray, output_path: str, reference_image_path: Optional[str] = None
):
...
def perform_inference_single_model(
    ...
    for i, img_file in enumerate(image_files, 1):
        ...
        if save_probability:
            ...
            save_prediction_as_nifti(prediction, str(prob_output), str(img_file))
        ...
        save_prediction_as_nifti(binary_prediction, str(binary_output), str(img_file))
...
def perform_ensemble_inference(
    ...
    for i, img_file in enumerate(image_files, 1):
        ...
        save_prediction_as_nifti(ensembled_pred, str(prob_output), str(img_file))
        ...
        save_prediction_as_nifti(binary_pred, str(binary_output), str(img_file))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Perform inference on medical images")
    ...
    parser.add_argument(
        "--output", type=str, required=True, help="Output directory for predictions"
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    mode_group.add_argument("--model", type=str, help="Path to single trained model")
    ...
    parser.add_argument(
        "--config", type=str, help="Configuration file (required for ensemble mode)"
    )
    parser.add_argument(
        "--model-dir",
        type=str,
