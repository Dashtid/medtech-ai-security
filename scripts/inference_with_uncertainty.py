...
def mc_dropout_inference(model: keras.Model, x: np.ndarray, n_samples: int = 30) -> dict:
...
def visualize_uncertainty(
    pet: np.ndarray, ct: np.ndarray, ground_truth: np.ndarray, results: dict, save_path: Path
):
    ...
    axes[1, 0].set_title("Probability Map", fontsize=12, fontweight="bold")
    axes[1, 0].axis("off")

    im = axes[1, 1].imshow(results["seg_std"][..., 0], cmap="viridis")
    axes[1, 1].set_title("Uncertainty (Std Dev)", fontsize=12, fontweight="bold")
    axes[1, 1].axis("off")
...
def main():
    ...
    parser.add_argument(
        "--slice-idx", type=int, default=None, help="Slice index (random if not specified)"
    )
    parser.add_argument("--n-samples", type=int, default=30, help="Number of MC samples")
    parser.add_argument(
        "--output", type=str, default="results/uncertainty", help="Output directory"
    )
    ...
    print(
        f"    Mean DICE: {2 * np.sum(results['seg_mean'][..., 0] * y) / (np.sum(results['seg_mean'][..., 0]) + np.sum(y)):.4f}"
    )
    ...
    print(
        f"    95% CI: [{results['surv_mean'] - 1.96*results['surv_std']:.4f}, {results['surv_mean'] + 1.96*results['surv_std']:.4f}]"
    )
